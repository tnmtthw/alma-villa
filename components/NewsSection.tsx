    ...
    <div className="pb-10">
      ...
      <section className="relative bg-[#23479A]">
        ...
        <div className="relative max-w-7xl mx-auto px-6 md:px-8 py-16">
          <h1 className="text-3xl md:text-4xl font-bold text-white mb-4">Community News</h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <input
              ...
              className="w-full rounded-lg border border-white/20 bg-white/10 px-4 py-3 text-white placeholder-white/70 shadow-sm focus:outline-none focus:ring-2 focus:ring-white/40 focus:border-white/70 backdrop-blur-sm"
            />
            <div className="md:col-span-2 flex flex-wrap items-center gap-2">
              {categories.map((c) => (
                <button
                  ...
                  className={`${activeCategory === c ? "bg-white text-[#23479A]" : "bg-white/10 text-white hover:bg-white/20"} border border-white/20 rounded-lg px-4 py-2 text-sm shadow-sm transition-all duration-200`}
                >
                  {c}
                </button>
              ))}
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-8 -mt-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-12">
          {filtered.map((item, index) => (
            <div
              key={item.id}
              className={`${index === 0 ? 'lg:col-span-2 lg:row-span-2' : ''} bg-white rounded-2xl shadow-sm hover:shadow-lg transition-all duration-300 cursor-pointer group border border-gray-100 overflow-hidden`}
            >
              {/* Image Section */}
              <div className={`relative ${index === 0 ? 'h-80' : 'h-48'} overflow-hidden`}>
                ...
                {item.priority && item.priority !== 'normal' && (
                  <div className="absolute top-4 left-4">
                    <span className={`px-3 py-1 rounded-full text-xs font-medium border ${getPriorityBadge(item.priority)?.class}`}>
                ...
                <div className="absolute top-4 right-4">
                  <span className={`px-3 py-1 rounded-full text-xs font-medium border-transparent ${getCategoryColor(item.category)}`}>
                    {item.category}
                  </span>
                </div>
              </div>
              ...
              <div className="p-6">
                ...
                {/* Title */}
                <h3 className={`font-bold text-gray-900 group-hover:text-[#23479A] transition-colors mb-3 ${
                ...
                {/* Read More Button */}
                <button className="inline-flex items-center px-4 py-2 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 bg-white hover:bg-[#23479A] hover:text-white hover:border-[#23479A] transition-all duration-200">
                  Read More
        ...
        <div className="mb-12">
          <h3 className="text-2xl font-semibold text-gray-900 mb-8 text-center">Popular Updates Today</h3>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
            {[
              ...
              <div key={i} className="bg-white rounded-xl p-4 shadow-sm hover:shadow-md transition-shadow cursor-pointer border border-gray-100">
                <div className="flex items-start gap-3">
                  <div className="flex-shrink-0 w-8 h-8 bg-[#23479A] text-white text-sm font-bold rounded-full flex items-center justify-center">
                    {i + 1}
                  </div>
        ...
        <div className="text-center mb-12">
          <button className="inline-flex items-center px-8 py-3 bg-[#23479A] hover:bg-[#23479A]/90 text-white rounded-lg font-medium transition-colors">
        ...
        <div className="bg-red-50 border border-red-200 rounded-2xl p-6">
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              <h4 className="font-semibold text-red-900 mb-1">Emergency Hotlines</h4>
